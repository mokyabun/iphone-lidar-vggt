...
configure_build_cache() {
  ...
  export UV_CACHE_DIR="${UV_CACHE_DIR:-${APP_CACHE_ROOT}/uv}"
...
configure_ccache() {
  if command -v ccache >/dev/null 2>&1; then
    ccache --set-config "cache_dir=${CCACHE_DIR}" >/dev/null 2>&1 || true
    ccache --max-size="${CCACHE_MAXSIZE}" >/dev/null 2>&1 || true
  else
    LOG_PREFIX="${LOG_PREFIX:-build-cache}" log "ccache not found; CUDA extension builds will run without compiler caching."
  fi
}
