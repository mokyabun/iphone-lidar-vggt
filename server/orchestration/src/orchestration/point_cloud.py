...
from .settings import env_bool, env_int, env_nonnegative_float
...
def temporal_consistency_filter(
    ...
    if (
        ...
        or not env_bool("OBJECT_TEMPORAL_FILTER", True)
    ):
        return points, colors

    voxel_size = env_nonnegative_float("OBJECT_TEMPORAL_VOXEL_METERS", 0.006)
    minimum_frames = env_int("OBJECT_TEMPORAL_MIN_FRAMES", 2)
    neighborhood = env_int("OBJECT_TEMPORAL_NEIGHBOR_CELLS", 1)
    ...
    minimum_kept = max(40, int(points.shape[0] * env_nonnegative_float("OBJECT_TEMPORAL_MIN_KEEP_RATIO", 0.55)))
...
def clean_point_cloud(
    ...
    *,
    ...
    if raw_count == 0 or not env_bool("POINT_CLOUD_CLEANUP", True):
        return points, colors_array, 0

    if voxel_size is None:
        voxel_size = env_nonnegative_float(
...
def _clean_with_open3d(
    ...
    minimum = env_int("POINT_CLOUD_CLEANUP_MIN_POINTS", 40)
    if len(pcd.points) >= minimum:
        pcd, _ = pcd.remove_statistical_outlier(
            nb_neighbors=min(env_int("POINT_CLOUD_OUTLIER_NEIGHBORS", 20), len(pcd.points) - 1),
            std_ratio=env_nonnegative_float("POINT_CLOUD_OUTLIER_STD_RATIO", 1.5),
        )
    ...
    if object_mode and len(pcd.points) >= minimum:
        ...
        if finite_distances.size:
            radius = max(
                voxel_size * 3.5,
                float(np.median(finite_distances)) * env_nonnegative_float("POINT_CLOUD_RADIUS_FACTOR", 4.0),
            )
            pcd, _ = pcd.remove_radius_outlier(
                nb_points=env_int("POINT_CLOUD_RADIUS_MIN_NEIGHBORS", 3),
                radius=radius,
            )
...
def _clean_with_numpy(
    ...
    if object_mode and points.shape[0] >= 40:
        ...
        if mad > 0:
            limit = median + env_nonnegative_float("POINT_CLOUD_FALLBACK_MAD_FACTOR", 8.0) * 1.4826 * mad
            keep = radial <= limit
