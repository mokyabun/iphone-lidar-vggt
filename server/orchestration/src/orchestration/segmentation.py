...
from .settings import env_float, env_int, object_settings
...
def object_mask(root: Path, frame: FrameRecord, depth: np.ndarray, allow_sam: bool = True) -> np.ndarray:
    backend = object_settings().mask_backend.lower()
...
def central_object_mask(depth: np.ndarray, intrinsics: np.ndarray | None = None) -> np.ndarray:
    if intrinsics is not None and object_settings().remove_dominant_plane:
...
def propagate_object_mask(
    ...
    source_points, _ = unproject_depth(
        ...
        stride=object_settings().propagation_stride,
    )
    ...
    depth_tolerance = object_settings().propagation_depth_tolerance_m
    ...
    radius = object_settings().propagation_dilation_px
    if radius > 0:
        try:
...
def _plane_removed_center_mask(depth: np.ndarray, intrinsics: np.ndarray) -> np.ndarray:
    ...
    plane_distance = settings.plane_distance_m
...
def _fit_dominant_plane(points: np.ndarray) -> tuple[np.ndarray, float] | None:
    ...
    settings = object_settings()
    iterations = settings.plane_ransac_iterations
    threshold = settings.plane_distance_m
...
def _sam3_center_mask(image_path: Path, depth_shape: tuple[int, int]) -> np.ndarray | None:
    ...
    model_name = object_settings().sam_model
...
def _mask_is_plausible(mask: np.ndarray) -> bool:
    ...
    settings = object_settings()
    min_ratio = settings.min_mask_ratio
    max_ratio = settings.max_mask_ratio
