...
from orchestration.io import read_image
from orchestration.models import FrameRecord
from orchestration.segmentation import resize_mask
...
def prepare_multiview_input(
    ...
    selected = _select_diverse_views(candidates, settings.max_images)
...
def _crop_rgba(image: np.ndarray, mask: np.ndarray) -> np.ndarray:
    ...
    padding = int(round(side * reconviagen_settings().crop_padding))
...
def _generate_mesh(input_dir: Path, output_path: Path) -> None:
    ...
    if worker_url:
        ...
        while True:
            error_path = str(settings.worker_error) if settings.worker_error else None
            ...
            try:
                ...
                try:
                    ...
                    message = payload.get("error") or payload.get("detail") or str(exc)
                except Exception:
                    message = str(exc)
    ...
    python_bin = settings.python_bin or sys.executable
    runner = Path(__file__).with_name("main.py")
...
def _convert_and_align_mesh(
    ...
    sample_count = min(reconviagen_settings().alignment_samples, max(500, mesh.faces.shape[0] * 2))
...
def _refine_metric_transform_icp(
    ...
    settings = reconviagen_settings()
    source = _finite_subsample(aligned_source, settings.icp_samples)
    target = _finite_subsample(target_points, settings.icp_target_samples)
    ...
    maximum_distance = settings.icp_max_distance_m or max(0.008, target_diagonal * 0.08)
    iterations = settings.icp_iterations
    current = source.copy()
...
def _support_plane_height(object_points: np.ndarray, scene_points: np.ndarray) -> float | None:
    ...
    vertical_band = np.abs(scene_points[:, 1] - object_bottom) <= reconviagen_settings().support_band_m
...
def _prepare_print_mesh(mesh, trimesh):  # noqa: ANN001
    ...
    settings = reconviagen_settings()
    if not settings.print_voxel_repair:
    ...
    pitch = settings.print_voxel_m or max(0.0015, minimum_extent / 180.0)
    try:
