...
from orchestration.geometry import keyframe_indices
from orchestration.io import read_image
from orchestration.models import FrameRecord
from orchestration.ply import write_point_cloud_ply
from orchestration.segmentation import resize_mask

from .manager import configure_huggingface_cache, ensure_vggt_repo
...
class VGGTService:
    ...
    def generate_from_image_dir(
        self,
        image_dir: Path,
        output_dir: Path,
        preserve_color: bool = True,
    ) -> tuple[Path, int]:
        return run_image_directory(image_dir, output_dir, preserve_color=preserve_color)
...
def run_image_directory(image_dir: Path, output_dir: Path, preserve_color: bool = True) -> tuple[Path, int]:
    suffixes = {".jpg", ".jpeg", ".png", ".webp"}
    image_paths = sorted(path for path in image_dir.iterdir() if path.suffix.lower() in suffixes)
    return _run_python_vggt(image_paths, output_dir, [], preserve_color, None)
...
def _run_python_vggt(
    ...
    if runtime.device == "cuda" and vggt_settings().empty_cache_after_run:
...
def _load_vggt_runtime() -> _VGGTRuntime:
    ...
    with _RUNTIME_LOCK:
        ...
        if device == "cpu":
            if not vggt_settings().allow_cpu:
        ...
        if device == "cuda":
            ...
            torch.backends.cudnn.benchmark = settings.torch_cudnn_benchmark
            try:
                torch.set_float32_matmul_precision(settings.torch_float32_matmul_precision)
            except Exception:
                pass
...
def _limit_vggt_frames(frames: list[FrameRecord]) -> list[FrameRecord]:
    max_images = vggt_settings().max_images
